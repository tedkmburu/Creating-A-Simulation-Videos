...
function createFeildVector(point)
{
    ...
    for (let i = 0; i < charges.length; i++) 
    {
        let distance = p5.Vector.dist(charges[i].position, point)
    ...
    if (drawVector)
    {
        let eVector = netForceAtPosition(point).mult(500)
        let scale = eVector.mag() / 50
        let finalPosition = eVector.copy();
