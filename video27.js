...
function displayEquiLines()
{
    ...
    equiLines.forEach(line => {
        ...
        beginShape();
        let voltage = voltageAtPoint(line[0])
        
        let color = (voltage > 0) ? "red" : "blue";

        if (voltage == 0) color = "white"
        stroke(color)
...
function getLinePoints(startingPosition, currentPosition, arrayOfPoints, iterations)
{
    if (arrayOfPoints == null) 
    {
        iterations = 0
        currentPosition = startingPosition
        arrayOfPoints = [startingPosition]
    }

    for (let i = 0; i < 100; i++) 
    {
        let nextPoint = netForceAtPosition(currentPosition)
        nextPoint.rotate(Math.PI / 2)
        nextPoint.setMag(0.125)
        currentPosition =  p5.Vector.add(nextPoint, currentPosition)
    }
    

    arrayOfPoints.push(currentPosition)

    let distance = p5.Vector.dist(startingPosition, currentPosition) 

    let stop = (distance < 15 && iterations > 100)

    if (stop) 
    {
        arrayOfPoints.push(startingPosition)
    }

    if (!stop && iterations < 2000) 
    {
        iterations++;
        getLinePoints(startingPosition, currentPosition, arrayOfPoints, iterations)
    }
